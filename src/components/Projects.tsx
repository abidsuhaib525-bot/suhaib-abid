  ...
  useEffect(() => {
    const ctx = gsap.context(() => {
      ...
      blocks.forEach((block: any, i: number) => {
        ...
        // 2. Image Parallax (Image moves slightly inside its container on scroll)
      ...
      <div className="relative z-10 max-w-[1440px] mx-auto px-6 md:px-12 lg:px-24 w-full">
        ...
        <div className="flex flex-col gap-32 md:gap-48 lg:gap-64 pb-32">
          {projectsData.map((project, index) => (
            <div key={index} className="project-block relative w-full flex flex-col lg:flex-row items-center gap-12 lg:gap-24 origin-top">
              
              {/* Left: Project Details */}
              <div className="w-full lg:w-[45%] flex flex-col gap-6 lg:gap-8 z-20">
                <div className="flex flex-col gap-3">
                  <span className="project-num text-white/80 font-mono text-3xl md:text-4xl">
                ...
                <h3 className="project-title text-4xl md:text-5xl lg:text-[56px] font-bold text-white uppercase tracking-tighter leading-[1] mt-2">
                  {project.title}
                </h3>
                ...
                <div className="project-cta mt-6">
                  <Link 
                    ...
                    className="group flex items-center gap-4 w-max px-6 py-3 rounded-full border border-white/10 bg-white/[0.02] hover:bg-white hover:text-black transition-all duration-500"
                  >
              ...
              {/* Right: Large Real Screenshot */}
              <div className="project-image-container w-full lg:w-[55%] relative group z-10">
                <Link href={`/projects/${project.slug}`} className="block relative w-full aspect-[16/10] rounded-xl overflow-hidden bg-[#050505] border border-white/[0.05] shadow-[0_0_40px_rgba(255,255,255,0.02)] transition-colors duration-700 group-hover:border-white/20 group-hover:shadow-[0_0_60px_rgba(255,255,255,0.04)]">
                  
                  {/* Image container for inner scale and parallax */}
                  <div className="absolute inset-[-5%] w-[110%] h-[110%] group-hover:scale-105 transition-transform duration-[1.5s] ease-out">
                    <Image 
                      src={`/projects/${project.slug}.png`} 
                      alt={project.title}
                      fill
                      sizes="(max-width: 768px) 100vw, 55vw"
                      className="project-image object-cover opacity-90 group-hover:opacity-100 transition-opacity duration-700"
                      onError={(e) => {
